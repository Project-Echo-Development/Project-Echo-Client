                ...
                for (int i1 = 0; i1 + this.scrollPos < this.field_146253_i.size() && i1 < i; ++i1) {
                    ...
                    if (chatline != null) {
                        ...
                        if (j1 < 200 || flag) {
                            ...
                            if (l1 > 3) {
                                int i2 = 0;
                                int j2 = (-i1 * 9) + (Echo.INSTANCE.getAddonManager().getAddon("cchat").isState() ? 14 : 0);
                                animate = (float) RenderingUtils.progressiveAnimation(animate, l + 4, 0.15);
                                RenderingUtils.drawRectangle((float) i2, (float) (j2 - 9), (float) i2 + animate, j2, l1 / 2 << 24);
                                if (Echo.INSTANCE.getAddonManager().getAddon("cchat").isState())
                                    RenderingUtils.drawRectangle(i2, j2 - 9, i2 + 1.5f, j2, Echo.INSTANCE.getClientColor());

                                String s = chatline.getChatComponent().getFormattedText();
                                GlStateManager.enableBlend();
                                this.mc.fontRendererObj.drawStringWithShadow(s, (float) i2 +
                                        (Echo.INSTANCE.getAddonManager().getAddon("cchat").isState() ? 4 : 0), (float) (j2 - 8), 16777215 + (l1 << 24));
