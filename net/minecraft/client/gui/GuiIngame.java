...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        for (val addon : Echo.INSTANCE.getAddonManager().getAddons())
            if(addon.isState())
                addon.displayHook();
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            float f = this.zLevel;
            this.zLevel = -90.0F;
            if (!Echo.INSTANCE.getAddonManager().getAddon("ch").isState()) {
                this.drawTexturedModalRect(i - 91, sr.getScaledHeight() - 22, 0, 0, 182, 22);
                this.drawTexturedModalRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 22 - 1, 0, 22, 24, 22);
            }else {
                animated = RenderingUtils.progressiveAnimation(animated, i - 91 + entityplayer.inventory.currentItem * 20, 0.8);
                RenderingUtils.drawBorderedRectangle(i - 91, sr.getScaledHeight() - 22, i + 91, sr.getScaledHeight() - 1,
                        1, new Color(0, 0, 0, 120).getRGB(), Echo.INSTANCE.getClientColor());
                RenderingUtils.drawRectangle((float) animated, sr.getScaledHeight() - 22,
                        (float) (animated + 23), sr.getScaledHeight() - 1, new Color(90, 70, 124, 195).getRGB());
            }
            this.zLevel = f;
