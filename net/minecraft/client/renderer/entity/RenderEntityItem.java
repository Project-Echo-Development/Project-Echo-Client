    ...
    private int func_177077_a(EntityItem itemIn, double p_177077_2_, double p_177077_4_, double p_177077_6_, float p_177077_8_, IBakedModel p_177077_9_)
    {
        ...
        if (item == null)
            ...
            if (flag || this.renderManager.options != null) {
                ...
                if (Echo.INSTANCE.getAddonManager().getAddon("ip").isState()) {
                    if (itemIn.onGround) {
                        double var = ((itemIn.posX + (itemIn.motionX * mc.timer.renderPartialTicks)) * 200)
                                + ((itemIn.posZ + (itemIn.motionZ * mc.timer.renderPartialTicks)) * 200);
                        GlStateManager.rotate((float) var, 0f, 1f, 0f);
                        if (!(item instanceof ItemBlock && !((ItemBlock) item).getBlock().isPassable(mc.theWorld, mc.thePlayer.getPosition())))
